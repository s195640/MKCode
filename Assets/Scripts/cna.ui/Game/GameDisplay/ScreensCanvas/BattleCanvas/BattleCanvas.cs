...
namespace cna.ui {
    public class BattleCanvas : BattleEngine {
        ...
        public void OnClick_RangeNext() {
            ...
            if (summoner) {
                AR.AcceptPanel("Warning!",
                    "Summoners are about to summon units, you will NOT be able to UNDO, do you want to continue?",
                    new List<Action<GameAPI>>() { OnClick_RangeNext_Summoners_Yes, (a) => { } },
        ...
        public void OnClick_RangeNext_Summoners_Yes(GameAPI ar) {
            List<MonsterMetaData> summonedMonsters = new List<MonsterMetaData>();
            List<string> monsterNames = new List<string>();
            List<int> allMonsters = new List<int>();
            allMonsters.AddRange(B.Monsters.Keys);
            B.Monsters.Values.ForEach(md => {
                CardMonsterVO c = (CardMonsterVO)D.Cards[md.Uniqueid];
                if (!md.Dead && c.MonsterEffects.Contains(UnitEffect_Enum.Summoner)) {
                    int monsterId = 0;
                    do {
                        monsterId = D.Scenario.GetRandomMonster(MonsterType_Enum.Brown);
                    } while (allMonsters.Contains(monsterId));

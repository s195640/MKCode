...
namespace cna.ui {
    public class LegendMonsterCanvas : UIGameBase {
        ...
        private void populateMonsterSlots(PlayerData pd) {
            for (int i = 0; i < green_CMG.Count; i++) {
                ...
                GreenMonsters[i].SetupUI(pd, D.Cards.Find(c => c.CardType == CardType_Enum.Monster && c.CardImage == green_CMG[i]).UniqueId, true);
            }
            for (int i = 0; i < grey_CMY.Count; i++) {
                GreyMonsters[i].gameObject.SetActive(true);
                GreyMonsters[i].SetupUI(pd, D.Cards.Find(c => c.CardType == CardType_Enum.Monster && c.CardImage == grey_CMY[i]).UniqueId, true);
            }
            for (int i = 0; i < violet_CMV.Count; i++) {
                VioletMonsters[i].gameObject.SetActive(true);
                VioletMonsters[i].SetupUI(pd, D.Cards.Find(c => c.CardType == CardType_Enum.Monster && c.CardImage == violet_CMV[i]).UniqueId, true);
            }
            for (int i = 0; i < brown_CMB.Count; i++) {
                BrownMonsters[i].gameObject.SetActive(true);
                BrownMonsters[i].SetupUI(pd, D.Cards.Find(c => c.CardType == CardType_Enum.Monster && c.CardImage == brown_CMB[i]).UniqueId, true);
            }
            for (int i = 0; i < white_CMW.Count; i++) {
                WhiteMonsters[i].gameObject.SetActive(true);
                WhiteMonsters[i].SetupUI(pd, D.Cards.Find(c => c.CardType == CardType_Enum.Monster && c.CardImage == white_CMW[i]).UniqueId, true);
            }
            for (int i = 0; i < red_CMR.Count; i++) {
                RedMonsters[i].gameObject.SetActive(true);
                RedMonsters[i].SetupUI(pd, D.Cards.Find(c => c.CardType == CardType_Enum.Monster && c.CardImage == red_CMR[i]).UniqueId, true);
            }
        }
    }
}

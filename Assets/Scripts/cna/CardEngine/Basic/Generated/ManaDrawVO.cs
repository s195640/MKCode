...
namespace cna {
    public partial class ManaDrawVO : CardActionVO {
        public ManaDrawVO(int uniqueId, Image_Enum avatar) : base(
            uniqueId,
            "Mana Draw",
            Image_Enum.CB_mana_draw,
            CardType_Enum.Basic,
            new List<string> { "You can use one additional mana die from the Source this turn.", "Take a mana die from the source and set it to any color except gold. Gain two mana tokens of that color. Do not reroll this die when you return it to the Source." },
